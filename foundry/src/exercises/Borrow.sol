...
contract Borrow {
    ...
    function approxMaxBorrow(address token) public view returns (uint256) {
        ...
        (, , uint256 availableBorrowsUSD, , ,) = pool.getUserAccountData(address(this));

        // Task 1.4 - Calculate the amount of token that can be borrowed
        return availableBorrowsUSD / price * (10 ** decimals) ;
    }
    ...
    function getVariableDebt(address token) public view returns (uint256) {
        ...
        address variableDebtTokenAddress = pool.getReserveData(token).variableDebtTokenAddress;
